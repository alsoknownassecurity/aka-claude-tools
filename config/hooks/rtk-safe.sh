...
# --- Credential-path guard (read commands only) ---
# Claude Code enforces Read(...) deny rules against file commands it RECOGNIZES
# (cat/head/tail/sed) but not against `rtk read`, which it doesn't know. So
# rewriting `cat ~/.ssh/id_rsa` → `rtk read ~/.ssh/id_rsa` would strip the user's
# Read(~/.ssh/**) deny AND — via the read-only rtk allowlist — auto-approve the
# read with no prompt. When a cat/head targets a credential path (the same set
# the secure base settings deny), leave it as-is: the original command stays
# recognized, so the deny rule still applies. Cost of a miss is one lost rewrite;
# cost of a wrong rewrite is a silently auto-approved secret read — so this errs
# toward NOT rewriting. Keep this list in sync with settings.base.json deny-reads.
CRED_PATH_RE='\.(ssh|gnupg|aws|azure|kube|pypirc|netrc|electrum|ethereum)($|[^[:alnum:]_])|\.npmrc($|[^[:alnum:]_])|/gcloud/|\.docker/config\.json|\.gem/credentials|\.git-credentials|/\.config/gh/|Library/Keychains/|Application Support/Electrum|/Electrum/|/Exodus/|Library/Ethereum/|/[Mm]eta[Mm]ask/|/[Pp]hantom/|/[Ss]olflare/|(^|[^[:alnum:]_.])\.env($|[^[:alnum:]_])'
case "$MATCH_CMD" in
  cat\ *|head\ *)
    if printf '%s' "$MATCH_CMD" | grep -qE "$CRED_PATH_RE"; then
      exit 0
    fi
    ;;
esac
...
# NOTE: rg/grep are deliberately NOT rewritten. Production data (12.7k commands):
# `rtk grep` choked on leading flags (-r/-E/-i/-A/-B) in 54% of rewrites — every
# one fell back to raw grep with zero savings — and rtk's own -v/-l/-m options
# shadow grep's (e.g. `grep -v pat` becomes "verbose match" instead of invert),
# silently returning wrong results. Removed 2026-06-11.
...
# Output the rewrite instruction. updatedInput is deliberately returned WITHOUT a
# permissionDecision: it modifies the command and lets the normal permission flow
# (allow/deny/ask rules, prompts) evaluate the rewritten input. Returning "allow"
# here would skip the permission prompt for every rewritten command (curl, docker,
# git push, …) — a silent grant this kit must not make.
jq -n \
