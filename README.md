...
| **startup-write-guard** | on | `PreToolUse` guard on `Bash`. Blocks commands that write/append/redirect into shell startup files (`~/.zshrc`, `~/.zshenv`, `~/.bashrc`, `~/.profile`, …) — the active counterpart to the secure-settings Edit/Write deny, which a Bash `echo >> ~/.zshrc` redirection would otherwise bypass. Reads are unaffected; intentional edits go through your own shell. Guidance, not a hard boundary. |
