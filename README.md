...
- **A small set of genuinely useful additions** layered onto any profile:
  secure defaults, per-surface egress guards (web queries + outbound Bash), a responsive status line, and a
...
| **leak-guard** | on | The **web** egress guard (`leak-guard.ts`, runs under **bun**). `PreToolUse` guard on `WebSearch`/`WebFetch` **and the SearXNG MCP tools** (`mcp__searxng__*` — so secure-deep-research's sensitive-topic path, routed through self-hosted SearXNG for privacy, is egress-scanned too; a no-op when no SearXNG server is configured). Blocks web queries/urls/prompts containing detected secrets (trufflehog, run local/detection-only — candidates are never sent to provider APIs for verification), Anthropic/AWS/GitHub/Slack/Stripe/OpenAI/webhook token shapes + SSH-key fragments (from the shared [`secret-patterns.json`](config/hooks/lib/secret-patterns.json)), and — if you configure them — your org's internal identifiers (`CT_EGRESS_PATTERNS`, read from the install-compiled `org-egress.json` sidecar). Secrets passed as `$VAR` references aren't literal text and sail through; only pasted-literal values block. **Fails closed** if its pattern file is missing/corrupt. **Requires `bun`** — a hard dependency: selecting leak-guard **aborts the install** if `bun` is absent, rather than shipping a default-on security guard silently disabled. (Bash egress is **command-guard**'s surface.) |
| **command-guard** | on | The **sole Bash egress guard** (bun). `PreToolUse` guard on `Bash`. Denies pipe-to-shell (`… \| sh/bash/zsh`) and **writes to shell startup files** (`~/.zshrc`, `~/.bashrc`, … — a persistence vector the secure-settings Edit/Write deny can't reach through a Bash `echo >> ~/.zshrc` redirection; legit alias writes go through `install.sh --alias`); and on **outbound** commands (`curl`/`wget`/`nc`/…, fast-gated — ~98% of Bash calls pay ~0 ms) denies a detected secret (trufflehog), an org internal-identifier match (`CT_EGRESS_PATTERNS` via the shared sidecar), and a credential *value* paired with the outbound tool (from [`secret-patterns.json`](config/hooks/lib/secret-patterns.json)); alerts on `nc`/`socat`/`sendmail`/env-dump/inline-interpreter. **Fails closed** on outbound if the pattern file is corrupt; a missing/malformed org sidecar leaves the org tier inactive without disabling the guard. **Requires `bun`** — a hard dependency: selecting command-guard **aborts the install** if `bun` is absent, rather than shipping a default-on security guard silently disabled (`leak-guard`, the web guard, requires `bun` too). |
...
| **Responsive status line** | on | Width-adaptive status line (`statusline.ts`, runs under **bun**): **repo + branch**, context, usage, and weather. Weather location auto-detects by IP (city-level); you can optionally **pin an exact location at install** (your entry is geocoded once via OpenStreetMap, only the resulting coordinates are stored locally — the text isn't kept and nothing is collected). Resolves its own config dir. Like command-guard it **requires `bun`** (a `.ts` can't degrade-run without it) — but it's **not** a security boundary, so that's an acceptable trade; deselect it if you don't want `bun`. |
...
cost of an accidental leak, they don't make exfiltration impossible. They're split
by **surface**: `leak-guard` (bun) guards **web** egress (`WebSearch`/`WebFetch`
and the SearXNG MCP tools); `command-guard` (bun) is the **sole Bash** egress guard. Both read one shared pattern
source ([`config/hooks/lib/secret-patterns.json`](config/hooks/lib/secret-patterns.json)
— both via `JSON.parse`) and are checked by a single test corpus
run against both, so their detection can't quietly drift apart. Org-marker patterns
(`CT_EGRESS_PATTERNS`) are consumed by both from one install-compiled JSON sidecar
(`org-egress.json`), constrained to a portable regex subset. Both guards run under **bun**
(a hard install dependency); a missing bun aborts the install rather than shipping a
default-on security guard silently disabled. Be honest about the edges:

- **The content scan is scoped to a fixed set of outbound tools.** `leak-guard` scans
  every web query; `command-guard` inspects a Bash command for secret *content* only
  when it names `curl`/`wget`/`nc`/`ncat`/`socat`/`fetch`. Commands that egress by other
  ...
  everyday Bash commands at ~0 ms — so the content scan is a backstop against careless
  paste-and-run, not a containment boundary. (Pipe-to-shell, `… | sh|bash|zsh`, is
  checked on **every** Bash command by `command-guard`, which is mandatory whenever the
  Bash guard is installed.)
...
- **Fail behavior is closed where it counts.** If the shared pattern file is missing
  ...
  shape allows it **loudly** (stderr). Since command-guard is the sole Bash guard,
  that single call goes unscanned — surfaced, not silent, and rare, but not backstopped
  by a second guard.
...
- **bun** (for **command-guard**, **leak-guard**, **rtk-safe**, and the **status line**) —
  all are `.ts` hooks, so selecting any requires `bun`; offered via `brew`/`npm` when
  selected. (No `curl … | bash` auto-run — our own command-guard blocks pipe-to-shell; if
  you have neither brew nor npm, install from https://bun.sh/install.) The status line is
  **not** a security boundary, so requiring `bun` there is acceptable; the two egress guards
  require it because a `.ts` hook can't degrade-run without it.
...
│   │                           #   rtk-safe (rtk), statusline.ts (bun), harness-pointer
│   ├── commands/wrap-up.md
