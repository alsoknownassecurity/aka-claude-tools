...
by **surface**: `leak-guard` (bash + `jq`) guards **web** egress (`WebSearch`/`WebFetch`
...
— bash via `jq`, TypeScript via `JSON.parse`) and are checked by a single test corpus
...
(`org-egress.json`), constrained to a portable regex subset so the two engines agree.
Keeping the web guard in pure bash is deliberate: a web-only install needs no bun, and
the web surface stays guarded even if bun is broken. Be honest about the edges:
...
- **bun** (for **command-guard** and the **status line**) — both are `.ts` hooks, so
  selecting either requires `bun`; offered via `brew`/`npm` when selected. (No
  `curl … | bash` auto-run — our own command-guard blocks pipe-to-shell; if you have
  neither brew nor npm, install from https://bun.sh/install.) The status line is **not**
  a security boundary, so requiring `bun` there is acceptable; the **web** egress guard
  (`leak-guard`) stays pure bash, so a web-only install still needs no `bun`.
